...
void main(void)
{
    ...
    //vec4 position = vec4(aPos,1.0f) + sin(0.02*time) * vec4(aNormal, 1.0f);
    vec4 in_Position = vec4(aPos,1.0f);
    in_Position.z += 0.1f*sin((-aPos.y + time)*20.0f);
    in_Position.x += 0.2f*sin((-aPos.y + time)*10.0f);
    ...
    vRefract[0] = refract( normalize( I ), worldNormal, mRefractionRatio * 1.0 ); // RED CHANNEL REFRACTION
    vRefract[1] = refract( normalize( I ), worldNormal, mRefractionRatio * 0.99 ); // GREEN CHANNEL REFRACTION
    vRefract[2] = refract( normalize( I ), worldNormal, mRefractionRatio * 0.98 ); // BLUE CHANNEL REFRACTION
